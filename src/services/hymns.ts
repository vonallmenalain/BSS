...
export async function importHymns(
  ...
  for (let offset = 0; offset < rows.length; offset += 400) {
    ...
    for (const row of chunk) {
      batch.set(
        doc(db, COLLECTIONS.hymns, String(row.number)),
        { number: row.number, title: row.title, updatedAt: serverTimestamp() },
        { merge: true },
      )
    }
    await batch.commit()
...
/** «2 – Der Geist aus den Höhen» bzw. «–», wenn noch nichts festgelegt ist. */
export function formatHymn(choice: HymnChoice | undefined | null): string {
  if (!choice || (!choice.number && !choice.title)) return '–'
  if (!choice.number) return choice.title
  return choice.title ? `${choice.number} – ${choice.title}` : `Nr. ${choice.number}`
}

/** Findet Lieder über Nummer oder Titel – für die Suche im Liedfeld. */
export function searchHymns(hymns: Hymn[], term: string, limit = 8): Hymn[] {
  ...
  return hymns
    .filter((hymn) =>
      Number.isFinite(asNumber) && String(asNumber) === text
        ? String(hymn.number).startsWith(text)
        : normalize(hymn.title).includes(text),
    )
